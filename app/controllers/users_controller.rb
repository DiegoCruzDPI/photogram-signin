class UsersController < ApplicationController
  def authenticate
    ...
    if user == nil
      ...
      #if there's a record, check to see if the password matches
  ...
  def create
    ...
    # conditional statment uses alert and notice to determin when the user inputs a specific task. It will give a specific meassage that
    # is unique depending on the situation.
    if save_status == true
      session.store( :user_id, user.id)
